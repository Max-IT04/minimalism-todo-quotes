...
export function generateUniqueId() {
    const timestamp = Date.now() // получаем текущую дату в числовом формате 
    const randomPart = Math.floor(Math.random() * 10000) // получаю первую рандом часть
    const randomPartTwo = Math.floor(Math.random() * 10000) // получаю вторую рандом часть
    return timestamp + randomPart + randomPartTwo // три части складываем
}

export function updateListTasks(tasks) {
